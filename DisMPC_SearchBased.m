...
for jj = 1:length(thorizon)
    ...
    tic;
    [U_opt_sc1, U_opt_sc2] = DMPCprocess(x0_sc1,x0_sc2,dt,N);
    u1 = U_opt_sc1(1:3,1);
    u2 = U_opt_sc2(1:3,1);
    t_runtime(jj) = toc;
    if jj > 1
...
DMPC_numerical.t = t(1:1440);
DMPC_numerical.a1 = a1(1:1440);
DMPC_numerical.a2 = a2(1:1440);
DMPC_numerical.ex1 = ex1(1:1440);
DMPC_numerical.ex2 = ex2(1:1440);
DMPC_numerical.ey1 = ey1(1:1440);
DMPC_numerical.ey2 = ey2(1:1440);
DMPC_numerical.i1 = i1(1:1440);
DMPC_numerical.i2 = i2(1:1440);
DMPC_numerical.Om1 = Om1(1:1440);
DMPC_numerical.Om2 = Om2(1:1440);
DMPC_numerical.u1 = u1(1:1440);
DMPC_numerical.u2 = u2(1:1440);
DMPC_numerical.ctrl1 = ctrl1(:,1:1440);
DMPC_numerical.ctrl2 = ctrl2(:,1:1440);
DMPC_numerical.dV1 = dV1(1:1440);
DMPC_numerical.dV2 = dV2(1:1440);
DMPC_numerical.RTN_2wrt1 = RTN_2wrt1(:,1:1440);

% saves simulation data
save('data/DMPC_numerical.mat','DMPC_numerical');
...
title('Distributed MPC Control Effort at Each Step');
xlabel('Simulation Time [s]');
...
title('Cumulative Distributed MPC Control Effort');
xlabel('Simulation Time [s]');
ylabel('Control Effort [m/s]');
...
% Run DMPC top level
function [U_opt_sc1, U_opt_sc2] = DMPCprocess(x0_sc1,x0_sc2,dt,N)

% Iterative DMPC
U0_1 = zeros(3*N,1); U0_2 = zeros(3*N,1);
toll = 100; Jold = 0; iter = 0;
while toll > 10 && iter < 50
    
    % Optimize over u1
    U_opt_sc1 = MPCprocess(x0_sc1,x0_sc2,U0_2,dt,N);
    
    % Optimize over u2
    U_opt_sc2 = MPCprocess(x0_sc2,x0_sc1,U_opt_sc1,dt,N);
    
    % Find cost with both
    Jnew = MPCsim(x0_sc1,x0_sc2,U_opt_sc1,U_opt_sc2,eye(6),0.1*eye(3),eye(6),N,dt);
...
function U_opt = MPCprocess(x0,x0ref,Uref,dt,N)
...
R = 0.001*eye(3);
Qf = (N^2)*eye(6);
Q(1,1) = 0.2*N / (0.0175 * (x0(1)+x0ref(1))/2);
Qf(1,1) = 0.2*N / (0.0175 * (x0(1)+x0ref(1))/2);
u_lb = -3*ones(3*N,1); % define deltaV lower bound [m/s]
u_ub = 3*ones(3*N,1); % define deltaV upper bound [m/s]
U0 = zeros(3*N,1); % define RTN control deltaVs [m/s]

% Run MPC
options = optimset('Display','off');
fun = @(U) MPCsim(x0,x0ref,U,Uref,Q,R,Qf,N,dt);
U_opt = fmincon(fun,U0,[],[],[],[],u_lb,u_ub,[],options);

end

% Simulate horizon cost
function totalcost = MPCsim(x0,x0ref,U,Uref,Q,R,Qf,N,dt)
...
for ii = 1:N-1
    ui = U((3*ii - 2):3*ii);
    uiref = Uref((3*ii - 2):3*ii);
    xicurr = xi_vec(:,ii);
    xirefcurr = xiref_vec(:,ii);
    xinext = nonlinear_dynamics(xicurr,ui,dt);
    xirefnext = nonlinear_dynamics(xirefcurr,uiref,dt);
    xi_vec = [xi_vec, xinext];
    xiref_vec = [xiref_vec, xirefnext];
    subcost = cost(xirefcurr,xicurr,ui,Q,R);
    subcostref = cost(xicurr,xirefcurr,uiref,Q,R);
    totalcost = totalcost + subcost + subcostref;
end
xicurr = xi_vec(:,N);
xirefcurr = xiref_vec(:,N);
subcost = termcost(xirefcurr,xicurr,Qf);
subcostref = termcost(xicurr,xirefcurr,Qf);
totalcost = totalcost + subcost + subcostref;
