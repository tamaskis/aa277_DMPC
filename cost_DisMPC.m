function J = cost_DisMPC( xk1, xk2, Q, R, P, N, u1, u2, dt )
%Inputs - 
%   f: dynamics function
...
for i = 1:N-1
    x1(:,i+1) = nonlinear_dynamics(x1(:,i),u1(:,i),dt);
    x2(:,i+1) = nonlinear_dynamics(x2(:,i),u2(:,i),dt);
end

%compute cost (summed up over prediction horizon)
J1 = sum(Q*(x1(:,1:end-1)-x2(:,1:end-1)).^2 + ...
         R*u1(:,1:end-1).^2) + ...
		 P*(x1(:,end)-x2(:,end)).^2;
J2 = sum(Q*(x1(:,1:end-1)-x2(:,1:end-1)).^2 + ...
         R*u2(:,1:end-1).^2) + ...
		 P*(x1(:,end)-x2(:,end)).^2;
J = J1+J2;
end
